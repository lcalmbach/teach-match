...
from school_management.models import (
    Person,
)  # Adjust the import according to your actual app and model names
import random
import string
    ...
    def create_users_for_persons(self):
        ...
        for person in persons:
            ...
            if person.is_teacher:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Added {username} to teachers group")
                )

            if person.is_candidate:
                candidates_group.user_set.add(user)
                self.stdout.write(
                    self.style.SUCCESS(f"Added {username} to candidates group")
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Created user {username} for person {first_name} {last_name}"
                )
            )
